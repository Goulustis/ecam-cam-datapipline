...
SCENE_PATH=data/checker                  # [REQUIRED] path/scene
TARG_DIR=data/formatted_checkers         # [REQUIRED] path to location to save the formatted dataset
RELCAM_PATH=$SCENE_PATH/rel_cam.json     # path to relative camera
TRIGGER_PATH=$SCENE_PATH/trigger.txt     # path to triggers
...
IMG_PATH=$SCENE_PATH/${SCENE}_recons/images        # path to original image
...
IMG_SCALE=2                                        # [REQUIRED] scale of used on colmap (eg if used 2x downsampled of original images, then this is 2)
