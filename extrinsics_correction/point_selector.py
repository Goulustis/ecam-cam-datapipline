...
class ImagePointSelector:
    ...
    def select_points(self, reselect=False):
        save_f = self.save_all_points(names_only=True)
        if osp.exists(save_f) and not reselect:
            return None
    ...
    def save_ref_img(self):
        ...
        cv2.imwrite(ref_f, self.composite_image) if not osp.exists(ref_f) else None
        print(ref_f)
