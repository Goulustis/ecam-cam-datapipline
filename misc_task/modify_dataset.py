...
def change_col_dataset(colcam: ColcamSceneManager, test_find:int, unused: list = None, sparce_fac: int = 3):
    ...
    if unused is None:
        unused_ids = set(colcam.image_ids[test_find:])
    else:
    ...
    unused_ids = unused_ids - set(val_ids) - set(test_ids)
    ...
    print("num train", len(train_ids), "num val", len(val_ids), "num test", len(test_ids))
...
def main():
    scene_dir = "/ubc/cs/research/kmyi/matthew/projects/ed-nerf/data/teddy_grass_v9_c1"
    ...
    train_rgb_ts = change_col_dataset(colcam, test_find=400, unused=None)
