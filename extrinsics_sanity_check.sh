source ~/.bashrc
PREP_EVENT_F=/ubc/cs/research/kmyi/matthew/backup_copy/raw_real_ednerf_data/work_dir/atrium_b2_v1/processed_events.h5
...
if [ ! -e "$EVS_IMG_TRIG_DIR" ]; then
    python extrinsics_correction/make_trig_eimgs.py -i $PREP_EVENT_F \
                                                    -t $TRIGGER_F \
                                                    -o $EVS_IMG_TRIG_DIR
fi

python extrinsics_correction/proj_viz_pnt_pairs.py --colmap_dir $COLMAP_DIR \
